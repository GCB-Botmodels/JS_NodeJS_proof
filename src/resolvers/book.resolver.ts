...
import { Length } from 'class-validator'; //sirve para hacer validaciones del tipo de datos que queremos ingresen a la base y las inconsistencias que podrían generar 
import { IContext, isAuth } from '../middlewares/auth.middleware';

//En los archivos resolver se crean las claseS que permitirán ejecutar Query (SELECT) y Mutation (CREATE, UPDATE, DELETE) 
...
class BookInput {
    ...
    @Field()
    @Length(3, 64) //Esta línea pone un mínimo y máximo de caracteres para el datos que se ingrese. Si no se cumple ello devuelve un error. 
    title!: string;
